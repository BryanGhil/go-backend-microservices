...
func (h *ProductGrpcHandler) GetSellerDashboardProducts(ctx context.Context, req *pb.GetSellerDashboardProductsRequest) (*pb.GetSellerDashboardProductsResponse, error) {
	// Pass the new pagination parameters
	products, totalCount, err := h.usecase.GetSellerDashboardProducts(
		ctx,
		req.GetLimit(),
		req.GetOffset(),
		req.GetSellerId(),
	...
	return &pb.GetSellerDashboardProductsResponse{
		Products:   pbProducts,
		TotalCount: totalCount,
	}, nil
}
